...
Locale is determined by QB_LOCALE env var (default: "en").
"""
...
def get_locale(root: Optional[Path]) -> str:
    """Get locale from QB_LOCALE env var or .claude/queen-bee/locale file."""
    env_locale = os.environ.get("QB_LOCALE")
    if env_locale:
        return env_locale
    if root:
        locale_file = root / ".claude" / "queen-bee" / "locale"
...
def get_local_context_dir(root: Optional[Path]) -> Optional[Path]:
    ...
    if root is None:
        return None
    local_dir = root / ".claude" / "queen-bee" / "contexts"
