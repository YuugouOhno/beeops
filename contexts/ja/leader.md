...
5. 自己批判レビュー
  ...
  ▼
6. CI 確認
  gh pr checks --watch でパスするまで待機
  ├─ 全チェック pass → 次ステップ
  └─ 失敗 → worker-coder に修正を依頼し、再度 CI 確認
  │
  ▼
7. 完了報告
...
Leader は Worker を起動する前に prompt file を書く。パス: `.beeops/tasks/prompts/worker-{N}-{subtask_id}.md`
...
`leader-{N}-summary.yaml` を `.beeops/tasks/reports/` に書き出す:
...
1. `.beeops/settings.json` から `github_username` を読み取る
2. Issue にコメントを投稿する:
