...
### 2.4 自律動作と Issue コメント質問

全エージェントは **ユーザーに直接質問しない**。AskUserQuestion は禁止。
判断に迷ったら自分で決定し、その理由をログ/レポートに記録する。
これにより、ユーザーが離席していても処理が止まらない。

ただし Leader は例外的に **GitHub Issue へのコメント** で質問できる。
曖昧な要件がある場合、Issue にコメントで質問を投稿し（settings.json の
`github_username` があれば @メンション付き）、回答を待たずにベストエフォートの
仮定で作業を進める。仮定はレポートに記録される。
...
- Issue を読んでサブタスクに分解
- **曖昧な要件があれば Issue にコメントで質問**（回答は待たず仮定で進める）
- Worker のプロンプトファイルを作成
- Worker を並列/順次に起動
- 品質を評価し、不十分なら再試行（最大 2 回）
- PR を作成し、サマリーレポートを書く（設計判断と代替案を必ず記録）
...
| `bo-task-decomposer` | Queen, Leader, Coder | タスクの詳細分解 |
...
| `bo-review-operations` | Code Reviewer | インフラ・運用設定レビュー |
| `bo-review-security` | Code Reviewer, Security Reviewer | セキュリティレビュー（Security Reviewer は常時発動） |
| `bo-review-process` | Reviewer | レビュープロセス改善 |

### 7.2 Worker ↔ スキル マッピング

各 Worker がどのスキルをいつ使うかはコンテキストファイルで明示されている:

| Worker | 使用するスキル | 備考 |
|--------|-------------|------|
| **Coder** | `bo-task-decomposer` + プロジェクト固有スキル | 複雑なサブタスクの再分解、コーディング規約等 |
| **Tester** | プロジェクト固有スキル | テスト基準、フィクスチャパターン等 |
| **Code Reviewer** | `bo-review-backend/frontend/database/operations` + `bo-review-security` | ファイル種類に応じたルーティング |
| **Security Reviewer** | `bo-review-security`（常時発動） + プロジェクト固有 | OWASP チェックリスト等 |
| **Test Auditor** | プロジェクト固有スキル | カバレッジ要件、品質ゲート等 |

全 Worker は実装開始/レビュー開始時に `ls .claude/skills/` でプロジェクト固有スキルを発見し、
該当するものがあれば Skill ツールで呼び出す。

**禁止スキル**: 全 Worker は `bo-dispatch`, `bo-leader-dispatch`, `bo-issue-sync` の使用が禁止されている（オーケストレーション系は Queen/Leader 専用）。
...
```yaml
# leader-42-summary.yaml（抜粋）
design_decisions:
  - decision: "JWT を採用"
    reason: "既存 API パターンに合致、ステートレス"
    alternatives:
      - option: "セッション Cookie"
        rejected_because: "サーバー側状態管理が必要、スケール時の問題"
clarifications:
  - question: "トークン有効期限は？"
    assumed: "15 分"
...
### 11.2 Code Reviewer の批判的スタンス

Code Reviewer は **「批判的敵対者」** として設計されている:

- デフォルトは懐疑。すべての変更に隠れた欠陥があると仮定
- 「動く」は不十分 → 正しく・効率的に・安全に・保守可能に動くかを問う
- AI 生成コードには追加の精査（もっともらしいが間違ったデフォルト値、浅いエラーハンドリング等）
- 何も問題を見つけないレビューは怠慢なレビューと見なす

### 11.3 反迎合チェック（Anti-Sycophancy）
...
| フィールド | 型 | デフォルト | 用途 |
|-----------|---|---------|------|
| `issues` | number[] | — | 処理する Issue 番号を限定 |
| `assignee` | string | `"all"` | `"me"` = 自分担当のみ |
| `skip_review` | boolean | `false` | レビューフェーズをスキップ |
| `priority` | string | — | 最低優先度フィルター |
| `labels` | string[] | — | 指定ラベルの Issue のみ処理 |
| `github_username` | string | — | Leader が Issue に質問する際の @メンション先 |
| `max_parallel_leaders` | number | `2` | 独立 Issue の Leader 並列実行上限 |
...
5. Queen Phase 2: タスクループ開始
   ...
   ├── 独立した Issue は max_parallel_leaders まで並列ディスパッチ（デフォルト 2）
   ├── status: dispatched に更新
   └── bo-dispatch スキル起動
   ↓
6. bo-dispatch: launch-leader.sh 実行
   ├── git worktree 作成: .beeops/worktrees/feat/issue-42
   ...
   ↓
7. Leader: Issue 分析
   ├── 要件に曖昧な点あり → Issue にコメントで質問（@メンション付き）
   ├── 回答を待たず仮定で進める（仮定はレポートに記録）
   └── サブタスク分解
...
9. Workers: 各ペインで並列実行
   ...
   ↓
10. Leader: 品質評価 → PR 作成（Design Decisions テーブル付き）→ サマリーレポート → Queen に signal
    ↓
